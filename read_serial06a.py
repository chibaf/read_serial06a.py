import serial, sys
import re
from pylab import *
import numpy

strPort = sys.argv[1]   # serial port
ser=serial.Serial(strPort,115200) 
print("connected to: " + ser.portstr)

file=sys.argv[2]  # file name
regex = re.compile('\d+')  # for extracting number from strings
f=open(file,"w+")
y=[0]*100
data=[];itime=0
datat=[]
av = [[0 for j in range(10)] for i in range(10)]
icng=0
while True:
  try:
    line = ser.readline()
    match = regex.findall(str(line))   # extracting number from strings
    sec=float(match[1])*60.0+float(match[2])+float(match[3])*0.1
    data.append(itime*0.1)    # set time and temps to data
    data.append(float(match[4]+"."+match[5]))
    data.append(float(match[6]+"."+match[7]))
    data.append(float(match[8]+"."+match[9]))
    data.append(float(match[10]+"."+match[11]))
    data.append(float(match[12]+"."+match[13]))
    data.append(float(match[14]+"."+match[15]))
    data.append(float(match[16]+"."+match[17]))
    data.append(float(match[18]+"."+match[19]))
    data.append(float(match[20]+"."+match[21]))
    data.append(float(match[22]+"."+match[23]))
    if (icng>10):
      for k in range(10):
        avv=numpy.average(av[k])
        avvv=abs(avv-data[k+1])/abs(avv)
        if (avvv>0.5):
          data[k+1]=avv
    if(match[3]=="0"):
      print(str(sec)+": "+str(data))
    f.write(str(sec)+", "); 
    flag=0
    for val in data:
      flag+=1
      f.write(str(val)); 
      if(flag<11): 
        f.write(", ")
    f.write("\n")
    x=range(0, 100, 1)
    y.insert(0, data[1])  # Tc No.6
    y.pop(100)
    for k in range(10):
      av[k].insert(0,data[k+1])
      av[k].pop(10)        
    clf()
    ylim(0,700)
    plot(x, y)
    pause(0.05) 
    itime=itime+1
    icng=icng+1
    data=[]
#    ser.flush()
  except KeyboardInterrupt:
    print(str(line))
    print ('exiting')
    break
ser.flush()
ser.close()
f.close()
